...
# ============================================================
: "${DATA_DIR:=/data}"
_env_uid="$(printenv UID 2>/dev/null || true)"
_env_gid="$(printenv GID 2>/dev/null || true)"
: "${RUN_UID:=${_env_uid:-1000}}"
: "${RUN_GID:=${_env_gid:-1000}}"
...
# ============================================================
# Bedrock: env -> server.properties mapping (write only if ENV non-empty)
# ============================================================
declare -A PROP_MAP=(
...
install_world_zip_from_s3() {
  ...
  # If zip contains worlds/, use it; otherwise assume extracted root contains world folders
...
cleanup_rcon_lock_on_boot() { rm -rf "${RCON_STOP_LOCK}" 2>/dev/null || true; }
acquire_rcon_stop_lock() { mkdir "${RCON_STOP_LOCK}" 2>/dev/null; }
...
rcon_exec() {
  ...
  while true; do
    if timeout "${RCON_TIMEOUT}" mcrcon -H "${RCON_HOST}" -P "${RCON_PORT}" -p "${RCON_PASSWORD}" "${cmd}"; then
      return 0
    fi
...
graceful_shutdown() {
  ...
  if wait_for_server_exit "${SHUTDOWN_WAIT_TIMEOUT}"; then exit 0; fi
  ...
  if wait_for_server_exit "${SHUTDOWN_TERM_WAIT}"; then exit 0; fi
...
run_server() {
  ...
  ( cd "${DATA_DIR}" && export LD_LIBRARY_PATH="${DATA_DIR}:${LD_LIBRARY_PATH:-}" && ./bedrock_server ) &
  SERVER_PID=$!
  wait "$SERVER_PID"
}
